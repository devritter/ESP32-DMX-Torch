...
void screen_main_render(u8g2_t *display, imu_xyz_t *acc_data, float pan, float tilt)
{
    ...
    do
    {
        ...
        u8g2_DrawStr(display, 0, 56, display_line_buffer);

        u8g2_DrawStr(display, 64, 32, "Output:");
        snprintf(display_line_buffer, sizeof(display_line_buffer), "Pan:  %6.2f", -pan);
        u8g2_DrawStr(display, 64, 40, display_line_buffer);
        snprintf(display_line_buffer, sizeof(display_line_buffer), "Tilt: %6.2f", -tilt);
        u8g2_DrawStr(display, 64, 48, display_line_buffer);
